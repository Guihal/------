...
function checkContrast() {
  ...
  for (const [tName, tHex] of Object.entries(textColors)) {
    for (const [bName, bHex] of Object.entries(bgColors)) {
      ...
      if (ratio < 4.5) {
        issues.push({ pair: `${tName} on ${bName}`, ratio: +ratio.toFixed(2), hex: `${tHex} on ${bHex}` });
      } else if (ratio < 7) {
        notes.push({ pair: `${tName} on ${bName}`, ratio: +ratio.toFixed(2), note: "AA pass, not AAA" });
      }
    }
  }
  return { issues, notes };
...
async function audit() {
  ...
  await page.screenshot({ path: path.join(SCREENSHOT_DIR, fullPageFile), fullPage: true });
  ...
  notes.push(`Heading structure: ${headings.map((h) => h.level).join(", ") || "none"}`);

  await browser.close();

  // Contrast
  const { issues, notes: contrastNotes } = checkContrast();
  contrastNotes.slice(0, 5).forEach((n) => notes.push(`Contrast note: ${n.pair} = ${n.ratio}:1 (${n.note})`));
  ...
  // Also write to file
  fs.writeFileSync(path.join(SCREENSHOT_DIR, "audit-result.json"), JSON.stringify(result, null, 2));
