...
function makeDeps(overrides?: {
	...
	return {
		...
		useCases: {
			createTask: vi
				.fn()
				.mockResolvedValue({ task: overrides?.createTask ?? mockTask }),
			completeTask: vi.fn().mockResolvedValue({
				task: overrides?.completeTask ?? {
					...mockTask,
...
describe("useTaskStore", () => {
	...
	it("completeTask updates task in list", async () => {
		...
		expect(deps.useCases.completeTask).toHaveBeenCalledWith(
			expect.objectContaining({ taskId: "t1", profileId: "p1" }),
		);
	...
	it("archiveTask updates task in list", async () => {
		...
		await store.archiveTask({
			taskId: "t1",
			profileId: "p1",
			now: "2026-05-19T00:00:00Z",
		});

		expect(deps.useCases.archiveTask).toHaveBeenCalledWith(
			expect.objectContaining({ taskId: "t1", profileId: "p1" }),
		);
	...
	it("groups computed calls resolveTaskList", () => {
		...
		expect(deps.useCases.resolveTaskList).toHaveBeenCalledWith(
			[mockTask],
			expect.any(Date),
		);
	});
