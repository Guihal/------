...
describe("migration-runner transaction", () => {
	...
	it("enforces foreign keys (FK constraint fails without profile)", async () => {
		...
		expect(() => {
			db.exec(
				"INSERT INTO tasks (id, profile_id, title, status, priority, complexity, complexity_source, created_at, updated_at) VALUES ('t1', 'p1', 'Task', 'active', 'normal', 'small', 'manual', '2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z')",
			);
	...
	it("enforces CHECK constraints on status", async () => {
		await applyMigrations(conn, migrations);
		db.exec(
			"INSERT INTO profile (id, display_name, created_at, updated_at) VALUES ('p1', 'Test', '2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z')",
		);

		expect(() => {
			db.exec(
				"INSERT INTO tasks (id, profile_id, title, status, priority, complexity, complexity_source, created_at, updated_at) VALUES ('t1', 'p1', 'Task', 'invalid', 'normal', 'small', 'manual', '2024-01-01T00:00:00Z', '2024-01-01T00:00:00Z')",
			);
	...
	it("enforces CHECK constraints on progression level", async () => {
		...
		expect(() => {
			db.exec(
				"INSERT INTO progression (profile_id, level, xp_total, updated_at) VALUES ('p1', 0, 0, '2024-01-01T00:00:00Z')",
			);
		}).toThrow(/CHECK constraint failed/);
	});
	...
	it("enforces CHECK constraints on progression xp_total", async () => {
		...
		expect(() => {
			db.exec(
				"INSERT INTO progression (profile_id, level, xp_total, updated_at) VALUES ('p1', 1, -1, '2024-01-01T00:00:00Z')",
			);
		}).toThrow(/CHECK constraint failed/);
	});
	...
	it("rolls back transaction on failed statement", async () => {
		...
		await expect(
			applyMigrations(conn, [...migrations, badMigration]),
		).rejects.toThrow();
