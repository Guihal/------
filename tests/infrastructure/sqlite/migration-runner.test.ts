...
if (typeof Bun === "undefined") {
	...
	describe("migration-runner", () => {
		...
		it("detects schema drift when checksum differs", async () => {
			...
			await expect(applyMigrations(conn, tampered)).rejects.toThrow(
				/schema drift/,
			);
		});
		...
		it("rejects duplicate versions", async () => {
			...
			await expect(applyMigrations(conn, dups)).rejects.toThrow(
		...
		it("times out on slow migration", async () => {
			...
			await expect(
				applyMigrations(slowConn, [fastMigration], 50),
		...
		it("rejects migration with transaction control", async () => {
			const bad = {
				version: 1,
				name: "bad",
				sql: "BEGIN; CREATE TABLE t(id INT); COMMIT;",
			};
			await expect(applyMigrations(conn, [bad])).rejects.toThrow(
