    ...
    return () =>
      h("div", { class: "profile-level", "data-testid": "profile-level" }, [
        h("div", { class: "profile-info" }, [
          h("span", { class: "profile-name", "data-testid": "profile-name" }, props.name),
          h("span", { class: "profile-level-badge", "data-testid": "profile-level-badge" }, `Lv. ${level.value}`),
        ]),
        h("div", { class: "xp-bar" }, [
          ...
          h("span", { class: "xp-text", "data-testid": "xp-text" }, `${progress.value} / ${XP_PER_LEVEL} XP`),
        ]),
      ])
  },
})
...
describe("ProfileLevel", () => {
  ...
  it("renders level 0 for 0 xp", () => {
    const wrapper = mount(ProfileLevel, { props: { name: "Alice", xp: 0 } })
    expect(wrapper.find('[data-testid="profile-level-badge"]').text()).toBe("Lv. 0")
  })

  it("renders level 1 for 1000 xp", () => {
    const wrapper = mount(ProfileLevel, { props: { name: "Alice", xp: 1000 } })
    expect(wrapper.find('[data-testid="profile-level-badge"]').text()).toBe("Lv. 1")
  })

  it("renders level 2 for 2500 xp", () => {
    const wrapper = mount(ProfileLevel, { props: { name: "Alice", xp: 2500 } })
    expect(wrapper.find('[data-testid="profile-level-badge"]').text()).toBe("Lv. 2")
  })

  it("renders xp progress text", () => {
    const wrapper = mount(ProfileLevel, { props: { name: "Alice", xp: 1234 } })
    expect(wrapper.find('[data-testid="xp-text"]').text()).toBe("234 / 1000 XP")
  })
