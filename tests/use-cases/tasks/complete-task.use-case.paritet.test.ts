...
function createConn(db: Database): SqliteConnection {
  return {
    ...
    run: async (s: string, v?: unknown[]) => { const stmt = db.prepare(s); const info = stmt.run(...(v ?? [])); stmt.finalize(); return { changes: Number(info.changes) } },
    query: async (s: string, v?: unknown[]) => { const stmt = db.prepare(s); const rows = stmt.all(...(v ?? [])) as unknown[]; stmt.finalize(); return { values: rows } },
  }
}

function makeTask(overrides: Partial<Task> & Pick<Task, "id" | "status">): Task {
  return { id: overrides.id, profileId: overrides.profileId ?? "p1", title: overrides.title ?? "Task", description: overrides.description ?? null, status: overrides.status, priority: overrides.priority ?? "normal", complexity: overrides.complexity ?? "small", complexitySource: "suggested", dueAt: overrides.dueAt ?? null, createdAt: overrides.createdAt ?? "2026-05-01T00:00:00Z", updatedAt: overrides.updatedAt ?? "2026-05-01T00:00:00Z", completedAt: overrides.completedAt ?? null, archivedAt: overrides.archivedAt ?? null }
}
...
describe("completeTask parity — memory vs sqlite", () => {
  it("memory: happy path", async () => {
    ...
    await seed(uow, task, { profileId: "p1", totalXp: 500, updatedAt: "2026-05-01T00:00:00Z" })
  ...
  it("memory: rollback on error", async () => {
    ...
    await seed(uow, task, { profileId: "p1", totalXp: 100, updatedAt: "2026-05-01T00:00:00Z" })
    const brokenUow: UnitOfWorkPort = { tasks: uow.tasks, profiles: uow.profiles, progressions: { async findById(id: string) { return uow.progressions.findById(id) }, async save() { throw new Error("db failure") } }, async run<T>(cb: () => Promise<T>): Promise<T> { return uow.run(cb) } }
    await expect(completeTask(brokenUow, { taskId: "t1", profileId: "p1", now: "2026-05-10T00:00:00Z" })).rejects.toThrow("db failure")
...
if (typeof Bun !== "undefined") {
  describe("completeTask parity — sqlite", () => {
    ...
    beforeEach(async () => { db = new Database(":memory:"); conn = createConn(db); await applyMigrations(conn, migrations); uow = new SqliteUnitOfWork(conn) })
