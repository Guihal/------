...
function makeTask(overrides: Partial<Task> & Pick<Task, "id" | "status" | "dueAt">): Task {
  return {
    id: overrides.id,
    profileId: "p1",
    title: "Task",
...
describe("resolveTaskList", () => {
  ...
  it("sorts by priority: high first", () => {
    const tasks: Task[] = [
      makeTask({ id: "t1", status: "active", dueAt: "2026-05-20T00:00:00Z", priority: "normal" }),
      makeTask({ id: "t2", status: "active", dueAt: "2026-05-20T00:00:00Z", priority: "high" }),
      makeTask({ id: "t3", status: "active", dueAt: "2026-05-20T00:00:00Z", priority: "low" }),
    ]
    const result = resolveTaskList(tasks, now)
  ...
  it("sorts no-deadline by createdAt desc", () => {
    const tasks: Task[] = [
      makeTask({ id: "t1", status: "active", dueAt: null, createdAt: "2026-05-01T00:00:00Z" }),
      makeTask({ id: "t2", status: "active", dueAt: null, createdAt: "2026-05-10T00:00:00Z" }),
    ]
    const result = resolveTaskList(tasks, now)
