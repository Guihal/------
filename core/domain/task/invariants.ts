...
export type StatusTransitionError =
  ...
  | { readonly kind: "invalid_transition"; readonly from: TaskStatus; readonly to: TaskStatus }

export type ValidatePriorityError =
  | { readonly kind: "empty_priority" }
...
export function canTransitionTo(
  ...
  if (!allowed.includes(nextStatus)) {
    return { ok: false, error: { kind: "invalid_transition", from: task.status, to: nextStatus } }
  }

  return { ok: true, value: undefined }
}

export function validatePriority(priority: string): DomainResult<TaskPriority, ValidatePriorityError> {
  if (priority === "") {
...
export function suggestComplexity(params: {
  ...
  const hasDescription = params.description !== null && params.description.length > 0
