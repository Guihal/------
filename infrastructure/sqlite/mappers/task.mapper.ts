...
const TASK_COMPLEXITY_VALUES: readonly TaskComplexity[] = [
	"tiny",
	"small",
	"medium",
	"large",
];
const TASK_COMPLEXITY_SOURCE_VALUES: readonly TaskComplexitySource[] = [
	"suggested",
	"manual",
];
...
function assertStringOrNull(value: unknown, field: string): string | null {
	...
	if (typeof value !== "string") {
		throw new TypeError(
			`Expected ${field} to be string or null, got ${typeof value}`,
		);
	}
	return value;
}
...
function assertEnum<T extends string>(
	...
	if (!allowed.includes(s as T)) {
		throw new TypeError(
			`Invalid ${field}: "${s}". Allowed: ${allowed.join(", ")}`,
		);
	}
	return s as T;
}
...
export function toDomain(row: unknown): Task {
	...
	return {
		...
		complexitySource: assertEnum(
			r.complexity_source,
			"complexity_source",
			TASK_COMPLEXITY_SOURCE_VALUES,
		),
