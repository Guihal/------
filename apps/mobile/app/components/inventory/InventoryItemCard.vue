  ...
  <article class="card" :class="`rarity-${item.rarity}`">
    ...
    <div class="body">
      ...
      <button
        ...
        type="button"
        :disabled="busy"
        :aria-label="item.equipped ? 'Снять ' + item.name : 'Надеть ' + item.name"
...
.card { display: grid; grid-template-columns: 5rem minmax(0, 1fr); gap: 0.75rem; padding: 0.8rem; border: 1px solid var(--stroke); border-radius: var(--radius-lg); background: var(--surface-card); box-shadow: var(--shadow-soft); }
...
button { justify-self: start; min-height: 44px; border: 0; border-radius: var(--radius-md); background: var(--accent); color: var(--bg); padding: 0 0.9rem; font-weight: 800; }
